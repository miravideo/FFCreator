...
class FFImage extends FFNode {
  constructor(conf = { animations: [] }) {
    ...
    } else if (conf.frame?.w && conf.frame?.h) {
      this.setFrame(conf.frame.x, conf.frame.y, conf.frame.w, conf.frame.h);
    }
  }
  ...
  async getPreview(time, { width, height, format='jpeg', timing='mat' }={}) {
    let matTime = time;
