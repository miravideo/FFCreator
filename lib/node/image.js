...
const { RGB2HSL, STR2RGB } = require('../utils/color');
...
class FFImage extends FFNode {
  ...
  async getPreview(time, { width, height, format='jpeg', timing='mat', rawFrame=false }={}) {
    ...
    if (timing === 'abs' || timing === 'rel') {
      matTime = this.materialTime(timing === 'rel' ? time + this.absStartTime : time).time;
      matTime = this.material.seekTime(matTime);
    }
  ...
  async preProcessing() {
    ...
    this.material.duration = this.duration;
    const { min, max, colormin, colormax, colorkey, colorsimilarity=0.2 } = this.conf;
    if (typeof(colorkey) === 'string' && colorkey.startsWith('#') && colorsimilarity &&
        (colorkey.length === 4 || colorkey.length === 7)) {
      const [hue, s, l] = RGB2HSL(...STR2RGB(colorkey));
      const sim = Math.round(255 * colorsimilarity);
      this.setCutoutColor(hue - sim, hue + sim);
    } else {
