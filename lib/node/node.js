...
class FFNode extends FFClip {
  ...
  updateAnimations() {
    ...
    } else if (this.conf.animate && typeof(this.conf.animate) === 'object') {
      this.addAnimate(this.conf.animate);
    }
    if (this.conf.effect) {
      if (typeof(this.conf.effect) === 'object' && this.conf.effect.name) {
        this.conf.effect = [this.conf.effect];
      }
      if (Array.isArray(this.conf.effect)) {
        this.conf.effect.map(effect => {
          this.addEffect(effect.name.split(','), effect.time, effect.delay);
        });
      } else if (this.conf.effect && this.conf.effectTime) {
