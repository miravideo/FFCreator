...
class FFNode extends FFClip {
  ...
  parseAudioMotion() {
    ...
    for (let [key, value] of Object.entries(aMotion)) {
      ...
      if (result && result.length > 1) {
        const newValue = value.replace(`{${result[1]}}`, `audioData['${result[1]}']`);
        attr[key] = eval(newValue);
      }
    }
    return attr
  }
  ...
  toAbs(key, newValue, _attr=null) {
    const oriValue = (_attr && _attr[key] !== undefined) ? _attr[key]: this.confAttr[key];
  ...
  async drawing(timeInMs, nextDeltaInMS) {
    ...
    if (texture && this.keyFrame) {
      const keyFrameAttr = this.keyFrameAttr(timeInMs / 1000, this);
      Object.assign(attr, keyFrameAttr)
    }
    if (this.confAttr?.amotion) {
