...
class FFFilter extends FFClip {
  ...
  async preProcessing() {
    // todo: load shader data by URL
    this.filter = this.parent.display.addFilter({ 
  ...
  get vert() {
    return XML.getValue(this.conf.vert);
  }

  get frag() {
    return XML.getValue(this.conf.frag);
  }

  get render() {
    return XML.getValue(this.conf.render);
  }
