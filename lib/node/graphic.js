...
class FFGraphic extends FFNode {
  ...
  createDisplay() {
    ...
    if (this.conf.asMask && blur) {
      const canvas = this.getCanvas(texture, blur);
      this.display = new Sprite(Texture.fromCanvas(canvas));
    } else {
  ...
  async getDisplay(time, opt) {
    ...
    const blur = Number(this.conf.blur);
    const canvas = this.getCanvas(texture, this.conf.asMask ? blur : 0);
    const display = new Sprite(Texture.fromCanvas(canvas));
    if (blur) display.blur = blur;
