...
class FFText extends FFNode {
  ...
  get confAttr() {
    ...
    conf.letterSpacing = this.px(conf.letterSpacing);
    return conf
  }
  ...
  get letterSpacing() {
    let lh = `${this.confAttr.letterSpacing || '0%'}`;
  ...
  get lineHeight() {
    ...
    return this.confAttr.lineHeight;
  }

  set lineHeight(lineHeight) {
    this.conf.lineHeight = this.vu(lineHeight, this.confAttr.lineHeight || '150%');
  }
  ...
  get padding() {
    let pd = `${this.confAttr.padding || '0%'}`;
    if (pd.endsWith('%') && !isNaN(pd.replace('%', ''))) {
      return this.fontSize * Number(pd.replace('%', '')) * 0.01;
    }
    return this.px(this.confAttr.padding);
  }

  set padding(padding) {
    this.conf.padding = this.vu(padding, this.confAttr.padding || '0%');
  }
  ...
  updateStyle() {
    ...
    } else if (this.confAttr.viewWidth && this.confAttr.viewHeight) {
      const x = this.px(this.confAttr.viewWidth) / this.display.width;
      const y = this.px(this.confAttr.viewHeight) / this.display.height;
