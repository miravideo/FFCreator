...
class FFText extends FFNode {
  ...
  set lineHeight(lineHeight) {
    ...
    if (lineHeight.endsWith('%')){
      this.conf.lineHeight = lineHeight;
    } else if (lh.endsWith('%') && !isNaN(lineHeight)) {
      this.conf.lineHeight = `${Math.round(100 * (lineHeight / this.fontSize))}%`;
    } else {
      this.setConfRpx('lineHeight', lineHeight);
    }
  }
  ...
  get styleStroke() {
    return {
      stroke: this.conf.stroke?.color || '#000',
  ...
  get styleShadow() {
    ...
    return {
      dropShadow: !!this.conf.shadow,
      dropShadowColor: this.conf.shadow?.color || '#000',
      dropShadowAlpha: Number(this.conf.shadow?.alpha) || 1.0,
  ...
  updateStyle() {
    const { width, fontSize, color, backgroundColor, wrap, align, lineHeight, 
      styleStroke, styleShadow } = this;
    const fontFamily = this.font?.fontFamily;
    const style = {
      fontFamily, fontSize, color, backgroundColor, align, lineHeight,
