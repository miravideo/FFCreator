...
class FFText extends FFNode {
  ...
  async preProcessing() {
    ...
    if (!isBrowser) {
      ...
      this.setColorKey();
      this.display.updateText(); // force update
      proxyObj.destroy();
    } else {
  ...
  setColorKey() {
    if (!this.conf.colorkey) return;
    const { colorkey, colorsimilarity=0.2 } = this.conf;
    this.display.texture.setCutoutColor(colorkey, colorsimilarity);
  }
  ...
  updateStyle() {
    const { fontSize, color, image, backgroundColor, wrap, align, lineHeight, styleStroke, styleShadow } = this;
    const wordWrapWidth = this.px(this.conf.width);
    const fontFamily = this.font?.fontFamily;
    const style = {
      fontFamily, fontSize, color, image, backgroundColor, align, lineHeight,
