...
class FFText extends FFNode {
  ...
  queuedFitSize(info) {
    return this.queue.enqueue(async () => {
      if (this.font?.fontFamily != this.conf.fontFamily) {
        if (!FONTS[this.conf.fontFamily]) {
          await this.setFont(this.conf.fontFamily);
          const fontFace = new FontFace(this.font.fontFamily, `url("${this.font.font}")`);
          const font = await fontFace.load();
          document.fonts.add(font);
          FONTS[this.conf.fontFamily] = this.font;
          await document.fonts.ready;
        } else {
          this.font = FONTS[this.conf.fontFamily];
        }
      }
