...
class FFTransition extends FFClip {
  ...
  refresh(delta) {
    ...
    if (this.filter) {
      if (params) this.filter.params = params;
      this.filter.ratio = this.width / this.height;
    }
  ...
  async preProcessing() {
    ...
    // 需要用Sprite来承接渲染
    this.prevTexture = RenderTexture.create(this.width, this.height);
    this.nextTexture = RenderTexture.create(this.width, this.height);
   ...
   createTransitionFilter(key) {
    ...
    this.filter = this.display.setTransition(source);
    if (this.conf.params) this.filter.params = this.conf.params;
  }
