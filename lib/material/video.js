...
class VideoMaterial extends ImageMaterial {
  constructor(conf) {
    super(conf);
    this.speed = Number(conf.speed) || 1.0;
    this.volume = Number(conf.volume);
  ...
  async seekTo(seekTime) {
    if (!this.$seekVideo) this.$seekVideo = this.$video.cloneNode(true);
    const video = this.$seekVideo;
    if (video.currentTime.toFixed(2) == seekTime.toFixed(2)) return video;
    return new Promise((resolve, reject) => {
