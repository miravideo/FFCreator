...
class VideoMaterial extends ImageMaterial {
  ...
  async play(seekTime) {
    ...
    $video.playbackRate = this.creator.playbackRate * this.speed;
    return new Promise(resolve => {
  ...
  async getFrameByTime(time, delta=0) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (delta > 0) {
        ...
        if (Math.abs(this.playerDelay) > 0.1) { // 时间差太多，就强制seek一下
          await this.seekTo(seekTime, K_PLAYER);
          // console.log('force seek!', this.holderId, this.playerDelay);
        }
