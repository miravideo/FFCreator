...
class FFCreator extends FFCon {
  constructor(conf = {}) {
    ...
    // materials
    this.mVIDEOS = [];
    this.analyser = new AudioUtil.Analyser();
  ...
  async timeUpdate(delta = 0, timeInMs = -1) {
    ...
    const mergedAudioFrame = new Float32Array(AUDIO_BUFFER_SIZE);
    for (const node of this.frameCallbacks) {
      if (typeof(node.getAudioFrame) !== 'function') continue;
      const singleAudioFrame = node.getAudioFrame(this.timer, AUDIO_BUFFER_SIZE);
      if (singleAudioFrame) {
        for (let i = 0; i < AUDIO_BUFFER_SIZE; i++) {
          mergedAudioFrame[i] += singleAudioFrame[i]
        }
      }
    }
  ...
  snapshot(opt={}) {
    ...
    this.render();
    const img = this.subImage(this.app.view, frame, opt);
    if (opt.next) {
