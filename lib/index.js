...
const Factory = {
  ...
  fromJson(json, cache, progress) {
    ...
    for (let childNode of childNodes) {
      ...
      if (!isBrowser && !childNode.active && !childNode.children.length) continue;
  ...
  getCreator(data, opt, progress) {
    const cache = [];
    const creator = this.fromJson({ ...data, ...opt }, cache, progress);
    return { creator, cache: Promise.all(cache) };
  },
  fromXml(xml, opt, progress) {
    ...
    return this.getCreator(data.children[0], opt, progress);
  },
  from(value, opt, progress=null) {
    ...
    if (value instanceof Object && value.type === 'canvas') {
      return this.getCreator(value, opt, progress);
    }
