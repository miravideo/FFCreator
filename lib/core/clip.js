...
class FFClip extends FFBase {
  ...
  getAudioFrame(timeInMs, bufferSize=1024) {
    ...
    for (let channel = 0; channel < this.audioBuffer.numberOfChannels; channel++) {
      ...
      const start = Math.round(timeInMs / 1000 * this.creator().getConf('audioSampleRate'));
      for (let i = 0; i < array.length; i++) {
        array[i] += (this.volume !== undefined) ? audioData[start + i] * this.volume : audioData[start + i];
      }
    }
   return array
  }
  ...
  get endTime() {
    ...
    if (this.type === 'audio') console.log('endTime', this.id, endTime);
