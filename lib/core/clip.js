...
class FFClip extends FFBase {
  ...
  annotate() {
    ...
    this.addTimelineCallback();
    this.onTime = (absTime, opDisplay=true) => {
  ...
  get realAbsEndTime() {
    return this.rt(this.parent.startTime + this.realEndTime);
  }
  ...
  setFilter() {
    if (!this.display) return;
    // remove prev filters
    if (Array.isArray(this.filters)) this.filters.map(x => this.display.removeFilter(x));

    let filterConf = this.conf.filter;
    if (!Array.isArray(filterConf)) {
      if (filterConf?.frag || filterConf?.render) filterConf = [filterConf];
      else return;
    }
    ...
    this.filters = [];
    for (const opt of filterConf) {
      let { frag, render } = opt || {};
      if (!frag && !render) continue;
      for (const k of ['vert', 'frag', 'render']) {
        opt[k] = parseHTML(opt[k]);
      }
      this.filters.push(this.display.addFilter(opt));
    }
  }
