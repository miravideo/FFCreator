...
class Synthesis extends FFEventer {
  ...
  addVideoFilter() {
    ...
    if (!this.videos || !this.videos.length) {
      if (this.stream) this.videoFilters.push(`[0:v]copy[vout]`);
      return;
    }
    ...
    if (this.stream) {
      this.videoFilters.push(`[${background}][0:v]overlay=0:0[vout]`);
    } else {
      this.videoFilters.push(`[${background}]copy[vout]`);
    }
  }
