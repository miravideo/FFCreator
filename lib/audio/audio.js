...
class FFAudio extends FFClip {
  ...
  async preProcessing(onprogress) {
    ...
    // todo: 延长0.2s
    this.buffer = await AudioUtil.getBuffer(this.material.path, 0.2, onprogress);
  ...
  toLoopFilter() {
    ...
    const r = 44100 * pitch;
    return `,asetrate=${r},aloop=-1:size=${dur * 44100}`;
  }
