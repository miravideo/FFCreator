...
class FFAudio extends FFClip {
  ...
  preProcessing() {
    ...
    this.howl = new Howl({ src: [path], format: ['mp3'], autoSuspend: false, rate: this.speed });
  ...
  async play(time) {
    ...
    this.howl.rate(this.speed, playId);
    console.log('set speed', this.id, this.speed, playId);
    // 用一种hack的方式设置大于1的音量
