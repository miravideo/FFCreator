...
class VideoHolder {
  ...
  static get(url, userId) {
    return new Promise((resolve, reject) => {
      VQ.enqueue(async () => { // 用Queue避免并发导致重复
        try {
          resolve(await this._get_(url, userId));
        } catch (e) {
          reject(e);
        }
      });
  ...
  static _get_(url, userId) {
    return new Promise((resolve, reject) => {
      const poolKey = userId.endsWith('preview') ? POOL_PREVIEW : POOL_DEFAULT;
      const vs = VPOOL[poolKey].filter(vh => vh.url === url);
      ...
      // 新建一个
      vh = new VideoHolder(vs[0] || url, userId);
